...
| Manychat 全页参考 | `docs/pencil-prototypes/reference/manychat-fullpage.png` | 2026-05-07 验证：该文件实际为 Cloudflare 安全验证页截图，不可作为 Manychat 视觉参考。 |
...
| Manychat Templates 顶部 DevTools 参考 | `docs/pencil-prototypes/reference/manychat-templates-devtools.png` | 2026-05-06 使用 DevTools MCP 真实页面截图，对应 Helo 第 4 屏模板 / 使用场景模块顶部。 |
| Manychat Templates 面板 DevTools 参考 | `docs/pencil-prototypes/reference/manychat-templates-devtools-panel.png` | 2026-05-06 使用 DevTools MCP 真实页面截图，对应 Helo 第 4 屏左列表 + 右预览面板。 |
...
### 9.6 DevTools 真实页面拆解：Manychat Templates / Use Cases

本节对应 Helo 第 4 屏，不是 Manychat 字面顺序的第四屏。2026-05-06 使用 DevTools MCP 打开 `https://manychat.com/`，定位到 Manychat 标题为 `From simple welcome messages to optimized sales & marketing funnels` 的 Templates / Use cases 模块后记录。

参考截图：

- `docs/pencil-prototypes/reference/manychat-templates-devtools.png`
- `docs/pencil-prototypes/reference/manychat-templates-devtools-panel.png`

#### 9.6.1 屏幕位置与整体结构

- 页面视口：DevTools 取样时约 `1440 x 735`，页面实际可视宽 `1425px`。
- section 文档位置：`top≈11450px`。
- section 尺寸：`1425 x 1251.5px`。
- section 背景：透明，承接页面默认白底。
- section padding：`160px 0 0`。
- 外层容器：`max-width≈1272px`，左边距约 `76.5px`，宽 `1272px`。
- 模块分两段：上方居中 intro，高度约 `311.5px`；下方模板演示面板，顶部距离 intro 约 `100px`，面板高度 `680px`。

#### 9.6.2 Intro 标题区

- icon：`80 x 80px`，居中，底部 margin `8px`。
- intro 容器：最大宽 `930px`，居中。
- 主标题：宽 `930px`，高约 `177px`，居中。
- 主标题字体：Manychat 使用 `ManyChatGravity, sans-serif`；字号 `72px`，字重 `400`，行高 `59.04px`，letter-spacing `-1.44px`，颜色 `rgb(0,0,0)`，文本居中。
- 副标题：`Launch fast...`，宽约 `478px`，居中；顶部 margin `24px`。
- 副标题字体：`16px`，字重 `400`，行高 `22.4px`，颜色 `rgb(0,0,0)`，文本居中。
- 进入动效：icon / 标题 / section 使用 `opacity + transform`，时长 `0.7s`，easing `cubic-bezier(0.43, 0.195, 0.02, 1)`。

#### 9.6.3 下方蓝色演示面板

- 面板容器：`1272 x 680px`。
- 面板背景：蓝色 `rgb(59,66,196)`。
- 面板布局：`position: relative; display:flex; overflow:hidden`，桌面端左右布局。
- 面板没有圆角，作为强色块横向切入页面；Helo 可保留强色整块，但颜色应替换成 Helo 的克制品牌强调色，避免直接使用 Manychat 蓝紫。
- 面板内有浅色网格背景层：absolute full fill，覆盖整块面板，用细网格增强“工具演示”感。
- 左侧白色列表卡：absolute，距面板左/上/下均 `20px`；尺寸 `285 x 640px`。
- 右侧预览区：从 `x≈361.5px` 开始，占 `987 x 680px`，`margin-left:285px`，最大宽 `calc(100% - 285px)`。
...
- 圆角：`16px`。
- padding：顶部 `32px`，左右 `24px`，底部 `24px`。
- 卡片内标题：`See it in action…`，宽 `237px`，高 `30px`。
- 卡片标题字体：`28px`，字重 `700`，行高 `30.24px`，letter-spacing `-1.12px`，左对齐，黑色。
- 列表容器：顶部 margin `20px`，宽约 `261px`，高度约 `379px`，纵向 flex，左右用 `-12px` 抵消 item padding。
- item 基础样式：宽 `261px`，圆角 `8px`，padding `12px 16px`，顶部间隔 `4px`，无阴影。
- item 普通背景：透明。
- item active 背景：`rgba(0,0,0,0.05)`。
- item 切换过渡：颜色 / 背景 / border / fill / stroke，`0.15s cubic-bezier(0.4,0,0.2,1)`。
- item 标题字体：`14px`，字重 `400`，行高 `19.6px`，letter-spacing `0.14px`，颜色黑色。
- item 小按钮文字：`CHECK IT OUT`，字体 `CoFo Sans Mono`，`12px`，字重 `400`，行高 `16.32px`，letter-spacing `0.12px`，大写；透明度 `0.4`。
- item 高度：普通单行 item 约 `63.9px`；长文本 item 约 `103.1px`。
- 左侧 CTA：位于卡片底部附近，`x≈120.5 y≈639.5`，尺寸 `237 x 48px`。
- CTA 样式：黑底白字，圆角 `100px`，padding `14px 20px`，字体 `CoFo Sans Mono`，`14px/20px`，字重 `400`，letter-spacing `0.14px`，大写。
- CTA 过渡：`background, transform`，`0.3s ease`。Manychat 的 hover 依赖内部滚动文字层，DevTools 直接 mouseenter 未读到明显颜色变化；Helo 可保留轻微 `translateY(-1px)` 或背景加深，不做夸张 hover。

#### 9.6.5 右侧预览区 / 动态模块

- 右侧区域尺寸：`987 x 680px`，relative。
- 内容居中：内部 flex `align-items:center; justify-content:center`。
- 手机视频容器：`300 x 568px`，水平居中，顶部约 `107px`。
- 实际 video：宽 `300px`，高约 `577.5px`，圆角 `30px`，`object-contain`，`loop`、`muted`、`playsInline`。
- 视频资源按 active 场景切换，例如 DevTools 中读到 `templates_03-v2.webm`，切换后也会加载其他 `templates_0x.webm`。
- 右侧动态切换动效：active media 使用 `opacity + transform`，时长 `0.4s`，easing `ease-out`。切换中间状态可见 `opacity≈0.29`、`translateY≈14px`，最终回到 `opacity:1; transform:none`。
- 右侧还有说明短文浮层，示例位置在右上，宽约 `190px`，白色文字，`14px/19.6px`。Helo 可改成当前模板的“导师说明 / 今日推进逻辑”，不要复制 Manychat 文案。

#### 9.6.6 Helo 第 4 屏内容映射建议

Manychat 的“营销自动化模板”应映射为 Helo 的“目标导师模板 / 使用场景”，保留左列表 + 右预览的理解方式。

| Manychat 元素 | Helo 替换 |
| --- | --- |
| Templates icon | 目标模板 / 指南针 / 日程节奏类 icon |
| `From simple welcome...` 大标题 | `从一个目标开始，Helo 会帮你拆成每天能做的事` |
| `Launch fast...` 副标题 | `选择一个目标模板，马上看到导师如何拆解、提醒和验收。` |
| `See it in action…` | `看看 Helo 怎么陪你推进` |
| 左侧 5 个营销场景 | 30 天英语口语、15 天面试冲刺、21 天早睡计划、7 天作品集冲刺、100 天健身入门 |
| `CHECK IT OUT` | `查看计划` / `预览导师` |
| 右侧手机自动化视频 | Helo 手机式导师对话或任务卡预览 |
| 右上白色说明浮层 | 当前目标的导师策略说明，例如“先定今日最小任务，再用问题验收完成质量。” |
| 黑色 `GET STARTED` | `开始一个目标` |

#### 9.6.7 Helo 实现规格建议

- 第 4 屏保持上方大标题 + 下方强色演示面板，不改成普通 2x3 模板卡。
- 桌面端 section padding 使用 `150px` 到 `160px` 顶部留白。
- 内容最大宽使用 `1260px` 到 `1280px`。
- intro 标题中文建议 `60px` 到 `72px`，行高 `0.9` 到 `1.0`，居中，宽度约 `900px`。
- 面板高度使用 `660px` 到 `700px`。
- 左卡宽保持 `285px` 到 `320px`，右预览占剩余宽度。
- 左卡必须是白底实体卡，右侧必须是大面积强色背景上的手机 / 任务演示，不要把每个模板做成同权重小卡。
- active item 用浅黑 `5%` 或 Helo 主色 `8%` 背景即可，避免强描边。
- 右侧预览应包含：目标名、导师身份、今日任务、验收问题、下一次提醒。核心是“导师 + 目标 + 今日行动 + 验收”，不是后台数据仪表盘。
- 切换交互如果实现：点击左侧 item 后右侧预览 `opacity 0 -> 1`、`translateY(14px -> 0)`，`0.4s ease-out`；左侧 active 背景 `0.15s` 过渡。
- 移动端可变为：intro 后蓝色面板内先显示横向可滚动场景 chips，再显示手机预览；左侧白卡不必强行保持绝对定位。

### 9.7 第 4 屏内容映射：Manychat Templates 到 Helo 目标场景

第 4 屏对应 Manychat 的 Templates / Use Cases 模块。Manychat 用这一屏让用户快速理解“我可以用这些自动化模板做什么”，Helo 应映射为“我可以用 Helo 推进哪些目标”。

这一屏不要做成普通模板卡片墙，而是保留 Manychat 的理解方式：

- 左侧：场景列表 / active tab
- 右侧：当前场景的导师陪跑预览
- 面板：强色背景 + 白色列表卡 + 手机式任务演示

| Manychat 参考元素 | 保留的设计作用 | Helo 内容替换 |
| --- | --- | --- |
| Templates / use cases section | 保留“代入使用场景”的作用 | 目标导师模板 / 典型目标场景 |
| Templates icon | 保留 chapter opener icon | 目标模板、指南针、日程、checklist 或 loop icon |
| `From simple welcome...` 大标题 | 保留“从简单到复杂都能开始”的表达 | `从一个目标开始，Helo 会帮你拆成每天能做的事` |
| `Launch fast...` 副标题 | 保留“选择模板快速启动”的节奏 | `选择一个目标模板，马上看到导师如何拆解、提醒和验收。` |
| `See it in action…` | 保留左卡标题 | `看看 Helo 怎么陪你推进` |
| 营销自动化场景列表 | 替换为目标场景列表 | 英语口语、面试冲刺、早睡计划、作品集冲刺、健身入门、自定义目标 |
| `CHECK IT OUT` | 保留小号 action label | `查看计划` 或 `预览导师` |
| 右侧手机视频 | 保留“当前模板演示预览” | 手机式 Helo 目标工作区：导师、目标、今日任务、验收问题、提醒状态 |
| 右上白字说明浮层 | 保留“当前场景策略说明” | 当前目标的导师策略，例如“先定今日最小任务，再用问题验收完成质量。” |
| `GET STARTED` | 保留底部 CTA | `开始一个目标` |
...
`从一个目标开始，Helo 会帮你拆成每天能做的事`

副标题：

`选择一个目标模板，马上看到导师如何拆解、提醒和验收。`
...
`学习、面试、习惯、作品集或健身，都可以被拆成每天能执行的小步。`
...
`你希望这个目标结束时，出现什么可验证结果？`

右上说明浮层：

`任何短期目标，都先被转成周期、任务和验收标准。`
...
1. Intro 先告诉用户：不用从空白开始。
2. 左侧列表让用户迅速找到自己的目标类型。
3. 右侧预览展示“导师 + 目标 + 今日任务 + 验收问题”的关系。
4. 底部 CTA 收束为“开始一个目标”，不要分散到多个复杂入口。

默认 active 场景建议使用 `30 天英语口语`，因为它最容易展示任务拆解、导师陪跑和提问验收。
...
### 10.2 DevTools 真实页面拆解来源

本节对应 Helo 第 5 屏，合并 Manychat 真实首页的三个连续收口模块：

1. Manychat 第 6 屏：3 steps
2. Manychat 第 7 屏：FAQ
3. Manychat footer

2026-05-07 本轮使用 DevTools MCP 重新打开 `https://manychat.com/` 时触发 Cloudflare Turnstile 验证，未能进入真实页面 DOM。以下规格以本文件 7.7 节保存的 2026-05-05 DevTools 真实页面测量为主，并结合本轮验证结论修正参考图可用性：`docs/pencil-prototypes/reference/manychat-fullpage.png` 当前实际是 Cloudflare 验证页，不可作为视觉依据。

关键 Manychat 测量记录：

- 3 steps section：`top≈13164px; height≈1163px; padding:160px 0`。
- FAQ section：`top≈14277px; height≈880px; padding:160px 0`。
- Footer：黑底，顶部白色透明分割线，`padding≈40px 0`。
- 通用内层最大宽：约 `1265px` 到 `1272px`。
- section intro：`80x80px` icon、`72px` 级居中 H2、`16px/22.4px` 副标题。
- FAQ 列表宽：约 `837px`，居中。

### 10.3 第 5 屏整体结构

Helo 第 5 屏不是把 Manychat 三个模块完整拉长照搬，而是压缩成一个连续收口区域：

1. 浅底三步开始
2. 黑底 FAQ
3. 黑底最终 CTA + 极简 footer

推荐高度与节奏：

- 三步开始：约 `760px` 到 `900px`，白色或浅暖白背景，`padding: 140px 0 120px`。
- FAQ + 最终 CTA + footer：合并为深色收口区，整体约 `900px` 到 `1100px`，黑底，FAQ `padding: 140px 0 96px`，最终 CTA `padding: 40px 0 96px`，footer `padding: 36px 0 44px`。
- 桌面端第 5 屏总高度可以约 `1700px` 到 `2000px`，但视觉上必须是一屏收口，不再分散成多个独立长 section。
- 背景转场：第 4 屏结束后进入浅底三步；FAQ 开始处切换为黑底，形成明确的最终收束。

### 10.4 三步开始视觉规格

Manychat 3 steps 的核心不是卡片墙，而是一个 chapter intro 后接三列流程。

Helo 保留：

- 顶部居中 icon：`80 x 80px`，可用 `--lime` 或 `--magenta` 做圆形底，图标用 check / route / spark 类简洁符号。
- H2：桌面端 `64px` 到 `72px`，中文行高 `0.92` 到 `1.0`，最大宽约 `900px`，居中。
- 副标题：`16px` 到 `18px`，行高 `1.4`，最大宽 `520px`，居中，顶部 margin `20px` 到 `24px`。
- 三列容器：最大宽 `1260px`，顶部 margin `88px` 到 `104px`，三列等宽，每列约 `390px` 到 `410px`。
- 三列之间用留白和细分割线建立流程感，不用厚重卡片阴影。
- 步骤编号：小号 uppercase 或圆形编号，建议 `01 / 02 / 03`，字号 `12px` 到 `14px`，字重 `500`，颜色可用 `rgba(18,18,18,0.48)`。
- 步骤标题：`20px` 到 `24px`，字重 `600`，行高 `1.25`。
- 步骤正文：`16px`，行高 `1.4`，颜色 `--muted`。
- 底部 CTA 组：居中，顶部 margin `56px` 到 `64px`；主 CTA 黑底白字，次 CTA 透明或浅描边黑字。

Helo 三步内容固定为：

| 步骤 | 标题 | 正文目的 | 可视化元素 |
| --- | --- | --- | --- |
| 01 | 创建你的 AI 导师 | 让用户选择导师风格，建立陪跑关系。 | 导师头像 / 语气标签 / 记忆开关 |
| 02 | 设定一个短期目标 | 把模糊愿望转为周期、频率和可验收结果。 | 目标输入框 / 周期选择 / 每日时间 |
| 03 | 每天执行并完成验收 | 每天拿到任务，完成后由导师追问确认，再进入下一天。 | 今日任务 / 验收问题 / 邮件提醒状态 |

推荐文案：

- H2：`三步，把目标交给 Helo 陪你推进`
- 副标题：`不用先整理复杂计划。先说出你想完成什么，Helo 会把它变成每天能执行、能验收、能被提醒的节奏。`
- 主 CTA：`开始一个目标`
- 次 CTA：`看看如何运作`

### 10.5 FAQ 视觉规格

Manychat FAQ 是黑底白字、透明 accordion 行，不是浅色卡片列表。Helo 第 5 屏应吸收这个强收口。

结构：

- FAQ section 背景：`--ink` 或纯黑。
- 顶部 icon：`80 x 80px`，居中，可用深底上的 `--lime` 小图标。
- H2：白字，`64px` 到 `72px`，中文行高 `0.92` 到 `1.0`，居中。
- 副标题可选：若使用，控制在一行到两行，`16px` 到 `18px`，颜色 `rgba(255,255,255,0.62)`。
- FAQ 列表宽：桌面端 `820px` 到 `860px`，居中，顶部 margin `72px` 到 `84px`。
- 每个问题行：透明背景，无卡片填充；高度约 `62px` 到 `72px`；上下用 `rgba(255,255,255,0.18)` 细线分隔。
- 问题文字：`16px` 到 `18px`，白色，字重 `400` 到 `500`。
- 答案文字：`16px`，行高 `1.5`，颜色 `rgba(255,255,255,0.6)`，展开后底部 padding `20px` 到 `28px`。
- 右侧箭头：简单 chevron 或 plus，尺寸 `20px` 到 `24px`；展开时旋转 `180deg` 或 `45deg`。
- 动效：展开 / 收起 `300ms cubic-bezier(0.43,0.195,0.02,1)`；箭头同一时长；不要弹簧感。

FAQ 控制在 4 条：

| 问题 | 答案方向 |
| --- | --- |
| Helo 和待办清单有什么区别？ | 待办清单只记录任务，Helo 会继续追问目标、拆成每日动作、在完成后验收，并在中断时提醒回到节奏。 |
| AI 怎么判断我是否真的完成？ | Helo 不只看勾选状态，会根据你的目标生成验收问题，让你用文字、结果或下一步反馈说明完成情况。 |
| 会不会提醒太频繁？ | 提醒应由目标周期和用户节奏决定，默认克制，只在关键节点邮件提醒，不做高压打扰。 |
| 可以创建多个导师或多个目标吗？ | 可以，但首页不应强调复杂管理；表达为每个目标都可以有自己的导师风格和推进节奏。 |

### 10.6 最终 CTA 与 Footer 规格

Manychat footer 紧接黑底 FAQ，用细分割线和多列链接收尾。Helo 需要更轻，因为当前首页核心目标是转化，不是展示大型 SaaS 站点地图。

最终 CTA：

- 背景：延续 FAQ 黑底。
- 位置：FAQ 列表之后，顶部 margin `56px` 到 `72px`。
- 宽度：内容最大宽约 `900px`，居中。
- 标题：`48px` 到 `64px`，白字，中文行高 `0.95` 到 `1.05`。
- 正文：`16px` 到 `18px`，颜色 `rgba(255,255,255,0.62)`，最大宽 `520px`，居中。
- CTA：主按钮使用 `--magenta` 背景白字，`height: 52px` 到 `56px`，圆角 `999px`，padding `0 28px`；可有一个透明白字次按钮，但不建议超过两个按钮。
- 推荐标题：`今天就让一个目标开始往前走`
- 推荐正文：`先创建一个导师，给它一个目标。Helo 会把第一天的行动、验收问题和提醒节奏准备好。`
- 推荐 CTA：`开始一个目标`

Footer：

- 背景：黑底。
- 顶部边线：`1px solid rgba(255,255,255,0.16)`。
- padding：`36px 0 44px`。
- 内容最大宽：`1260px`。
- 结构：左侧品牌名 / 一句短说明，右侧 2 到 4 个链接。
- 链接字号：`12px` 到 `14px`，颜色 `rgba(255,255,255,0.56)`，hover 转白或出现细下划线。
- 不放社交证明、客户 logo、复杂 sitemap、假法律链接。
- 可用链接：`Product`、`How it works`、`FAQ`、`Get started`。
- copyright 可写产品名与年份，但不要制造不存在的公司规模背书。
...
- 真实用户数、客户 logo、收益数据
- 医疗、心理、法律、财务结果承诺
- “保证完成目标”“保证养成习惯”等绝对化承诺
- 复杂 pricing / enterprise 链接
- 大型多列 footer 抢走 CTA 注意力

### 10.8 第 5 屏验收标准

1. 三步开始、FAQ、最终 CTA、footer 必须形成连续收口，不再像四个割裂模块。
2. 三步开始必须是 Manychat 式 chapter intro + 三列流程，不是普通功能卡网格。
3. FAQ 必须使用黑底透明行 accordion 风格，不做浅色卡片化 FAQ。
4. FAQ 控制在 4 条以内，优先回答转化阻力。
5. 最终 CTA 必须明显，但不制造廉价促销感。
6. Footer 必须极简，视觉权重低于最终 CTA。
7. 桌面端 `1440x900` 下，进入黑底区域后应能同时感到 FAQ 和最终 CTA 是同一个收尾系统。
8. 移动端只需保证三步纵向堆叠、FAQ 可读、footer 链接不重叠；不要为了移动端重写桌面结构。

### 10.9 第 5 屏内容映射：Manychat 收口模块到 Helo 转化闭环

第 5 屏合并 Manychat 的 3 steps、FAQ 和 footer。Helo 不应把它们做成三个割裂 section，而应压缩成一个连续转化收口：先告诉用户如何开始，再回答主要疑虑，最后给出一个明确 CTA。

| Manychat 参考元素 | 保留的设计作用 | Helo 内容替换 |
| --- | --- | --- |
| 3 steps section | 保留“开始门槛很低”的说明作用 | 三步开始一个目标：创建导师、设定目标、每天执行并验收 |
| 3 steps icon + 大标题 | 保留 chapter opener 节奏 | 用目标 / route / check icon，引出 `三步，把目标交给 Helo 陪你推进` |
| 三列步骤 | 保留横向流程理解方式 | `创建你的 AI 导师`、`设定一个短期目标`、`每天执行并完成验收` |
| 3 steps 底部双 CTA | 保留主次行动 | 主 CTA `开始一个目标`，次 CTA `看看如何运作` |
| 黑底 FAQ | 保留强收口和转化兜底 | 回答待办清单区别、AI 验收方式、提醒频率、多导师 / 多目标 |
| FAQ 透明 accordion 行 | 保留克制、成熟的交互 | 不做浅色卡片 FAQ，问题展开后只显示必要答案 |
| Footer 黑底细分割线 | 保留页面结束感 | 极简品牌、短说明、少量导航链接 |
| 大型 SaaS footer sitemap | 不保留 | Helo 当前不展示复杂公司链接、客户资源和大型站点地图 |
| 最终 CTA | 强化 | FAQ 后追加单个强 CTA，让用户立刻创建目标 |
...
`不用先整理复杂计划。先说出你想完成什么，Helo 会把它变成每天能执行、能验收、能被提醒的节奏。`
...
`创建导师，设定目标，然后每天跟着任务执行。完成后，Helo 会用问题帮你确认质量。`
...
`选择温柔陪跑、高效教练、理性分析或严格督促等导师风格，让这个目标有一个固定的陪跑对象。`
...
`输入你想完成的目标、周期和每天可投入的时间。Helo 会把模糊愿望拆成阶段计划和每日任务。`
...
`每天收到今日任务，完成后回答导师的验收问题。中断时，Helo 会用克制的邮件提醒你回到节奏。`
...
FAQ 只保留 4 条，优先回答转化阻力。答案需要克制、具体，不要写成营销长文。
...
- 不承诺 AI 判断永远准确。
- 不承诺提醒能保证用户坚持。
- 不把 Helo 写成心理治疗、医疗建议或职业成功保证。
- 不引入价格、企业版、团队协作等当前首页未承接的信息。

### 10.12 最终 CTA 与 Footer 内容映射
...
`先创建一个导师，给它一个目标。Helo 会把第一天的行动、验收问题和提醒节奏准备好。`
...
`AI 目标导师，把短期目标拆成每天能执行、能验收、能被提醒的节奏。`
...
- 不放虚假公司规模。
- 不放复杂 sitemap。
- 不放未准备承接的 pricing、enterprise、case studies。

### 10.13 第 5 屏叙事重点

第 5 屏的叙事顺序应为：

1. 三步开始先降低行动门槛：用户不需要先想清楚完整计划，只要创建导师并输入一个目标。
2. FAQ 处理最后疑虑：它和待办清单有什么不同、AI 如何验收、提醒是否打扰、是否支持多个目标。
3. 最终 CTA 把用户带回一个动作：开始一个目标。
4. Footer 只做轻收尾，不再开启新的信息分支。

第 5 屏的核心句子是：

`从一个目标开始，Helo 会把它变成每天能执行、能验收、能被提醒的节奏。`
